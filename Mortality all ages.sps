* Syntax to prepare All-cause mortality for all ages indicator.
...
*Extracting data on deaths of all age Scottish residents, excluding records with unknown sex and age.
GET DATA
  /TYPE=ODBC
  /CONNECT=!connect
...
save outfile = 'Raw Data/Prepared Data/deaths_allages_dz11_raw.sav'.
...
save outfile = 'Raw Data/Prepared Data/deaths_allages_dz01_raw.sav'.

dataset close basefile.

add files file='Raw Data/Prepared Data/deaths_allages_dz01_raw.sav'
/file='Raw Data/Prepared Data/deaths_allages_dz11_raw.sav'.
execute.
...
save outfile= 'Raw Data/Prepared Data/DZ_deaths_allages_IR_raw.sav'.
...
!stdrate data=deaths_allages_dz11 domain='Deaths, Injury and Disease'   type=stdrate time='3-year aggregate' yearstart=2002 
yearend=2016 pop='DZ11_pop_allages_SR' epop_age=a.

*Excluding IZ level from 2002 to 2011 as there is no population data available or is not complete for the 3yr aggregate (note 2011 is midpoint of 3 year time period so would be missing 2010 data). .
get file='Output/deaths_allages_dz11_formatted.sav'.

select if not (substr(code, 1, 3) = 'S02' and range(year, 2002,2011)).
frequencies year.

save outfile='Output/deaths_allages_dz11_formatted.sav'.
...
*Datazone 2011.
!standardisation data =deaths_allages_dz11 domain='Deaths, Injury and Disease' ind_id =20103 year_type = calendar 
min_opt = 480412 max_opt = 800000 profile = HN Epop_total=200000.
