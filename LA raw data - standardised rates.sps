*Syntax to manipulate the raw data for the ScotPHO profiles indicators, where the data is available at LA level and 
   the output is standardised rates.
*This syntax is a macro that aggregates the data to the different geographical levels required (LA, HB and Scotland),
...
*Joanna Targosz, 16 September 2016.
*Neil Davies, 22 December 2016 - added 'epop_age' and 'pop' criteria to make it suitable for multiple indicators.
*Jaime Villacampa Dec17. Adding <16 population case.

*Part 1 - read in raw data and add in lookup info.
*Part 2 - Aggregate up to get figures for each geographical area (i.e. LA, HB).
...
sort cases by year LA sex_grp age_grp.

*standardise area variables to enable merge.
alter type LA (a27) sex_grp (f8) age_grp (f2).
...
*save as csv to enable checking.
 save translate outfile= !quote(!concat('/conf/phip/Projects/Profiles/Data/Indicators/',!unquote(!domain),'/Output/',!unquote(!data),'_check3.csv'))
   /type=csv  /replace   /drop epop_age.
